...
driver = webdriver.Chrome(r"C:/Users/rmsdu/OneDrive/문서/GitHub/2023_OSSP/chromedriver.exe")    # 구글 웹드라이버를 사용
driver.implicitly_wait(3)
driver.get("https://www.google.co.kr/imghp?hl=ko&ogbl")                 # 구글 이미지 페이지로 들어간다.
element = driver.find_element(By.NAME, 'q')                                # 검색창
element.send_keys("cocacola can")                                          # 키보드 입력값
element.send_keys(Keys.RETURN)                                             # enter key

#element.send_keys("sprite can")                                            # 키보드 입력값
#element.send_keys("pocari sweat can")                                      # 키보드 입력값
#element.send_keys("데자와 캔")                                             # 키보드 입력값
#element.send_keys("레쓰비 캔")                                             # 키보드 입력값
#element.send_keys("fanta orange can")                                      # 키보드 입력값
    ...
    if new_height == last_height: # 끝까지 내려진 상황
        try :
            driver.find_element(By.XPATH,
                                '//*[@id="islmp"]/div/div/div/div/div[1]/div[2]/div[2]/input').click()  # 결과 더보기 선택
...
for image in images : 
    try : 
        image.click() 
        time.sleep(10)
        imgUrl = driver.find_element(By.XPATH_SELECTOR, '/html/body/div[2]/c-wiz/div[3]/div[2]/div[3]/div/div/div[3]/div[2]/c-wiz/div/div[1]/div[1]/div[3]/div/a/img').get_attribute("src")
        # 다운받을 이미지 창 띄우기
        # src : 재생할 미디어 파일의 URL을 명시    # 다운받을 이미지 창 띄우기
        ...
        urllib.request.urlretrieve(imgUrl, "coca" + str(count) + ".jpg") # 지정한 파일이름으로 저장
